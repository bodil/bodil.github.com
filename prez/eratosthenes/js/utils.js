(function() {
    ...
    Slide.prototype = {
        ...
        getSieveNode: function() {
            return this._sieveNode;
        },
        buildNext: function() {
            if (this._sieve) {
                return this.sieveRun();
            }
        ...
        sieveRun: function() {
            var next = query("td.next", this._sieve);
            if (!next) return false;
            var prime = parseInt(next.innerHTML, 10);
            addClass(next, "prime");
            removeClass(next, "next");
            if (prime > 8) {
                var mark = function(marklist) {
                    if (marklist.length) {
                        addClass(marklist[0], "prime");
                        setTimeout(function() { mark(marklist.slice(1)); }, 100);
                    }
                }
                var marklist = queryAll("td.next", this._sieve);
                marklist.map(function(td) { removeClass(td, "next"); });
                setTimeout(function() { mark(marklist); }, 100);
                return true;
            }
            var crosslist = [];
            for (i = prime * 2; i <= 64; i += prime) {
                var flarp = query("td.next.n" + i, this._sieve);
                if (flarp) {
                    crosslist.push(flarp);
                    removeClass(flarp, "next");
                }
            }
            if (crosslist.length) {
                var cross = function(crosslist) {
                    if (crosslist.length) {
                        addClass(crosslist[0], "crossed");
                        setTimeout(function() { cross(crosslist.slice(1)); }, 100);
                    }
                };
                setTimeout(function() { cross(crosslist); }, 500);
            }
            return true;
        }
    };
    ...
    queryAll("div.sieve").forEach(function(div) {
        ...
        for (i = 0; i < 8; i++) {
            ...
            for (j = 0; j < 8; j++) {
                ...
                column.innerHTML = "" + number;
                addClass(column, "n" + number);
                if (number == 1) addClass(column, "prime");
                else addClass(column, "next");
